...
# A function that takes in a web page as a string, finds the first url,
# and returns that url and the ending position of the url.
#
...
def getNextTarget(page):
    start_link = page.find(' href="http')
...
def crawlWeb(seed, max_depth):
    ...
    while to_crawl and depth <= max_depth:
        ...
        if url not in crawled:  # Check if we have already crawled this page
            page = getPage(url)     # Get the page as a string
            new_links = getAllLinks(page)   # Crawl the page
            # Add new_links to the next_depth list so we have to finish current depth level
            # before moving on
            union(next_depth, new_links)
    ...
    return crawled  # return list of urls of pages crawled.

# Kick things off with a seed page.
print crawlWeb("http://xkcd.com/353", 1)


# # This function opens an html file we have in this directory and gets all
# # its links.
#
# def getLinksFromFile():
#     # Open up the html file and read the file's contents as a string
#     in_file = open('testWiki.html', 'r')
#     web_page = in_file.read()

#     # Get all links from the page
#     links = getAllLinks(web_page)

#     # If we have any links, print them to file
#     if len(links) > 0:
