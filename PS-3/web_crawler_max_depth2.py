# THIS IS THE solution given by the instructor. I think it is better than my own.
#
# 
...
# So this way finishes an entire level of depth before moving on to the next level. 
# i.e. we crawl the seed page at depth = 0, after that page we are out of links in
# tocrawl, so we fill tocrawl list with the next_depth list(depth level 1 list). 
# Then the while loop cycles through ALL level 1 links (until the tocrawl list is empty
# again). Then we fill tocrawl list with depth level 2. 
